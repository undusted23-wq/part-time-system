import { Link, Outlet } from "react-router-dom";
import { Button } from "@/components/ui/button";
import {
  ActivityIcon,
  BellIcon,
  BriefcaseIcon,
  BuildingIcon,
  ClipboardIcon,
  CogIcon,
  FileTextIcon,
  HomeIcon,
  LogOutIcon,
  MessageSquareIcon,
  ShieldIcon,
  StarIcon,
  UserIcon,
  UsersIcon
} from "lucide-react";
import { Separator } from "@/components/ui/separator";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Badge } from "@/components/ui/badge";

export default function AdminLayout() {
  return (
    <div className="grid min-h-screen grid-cols-[240px_1fr] bg-muted/5">
      <aside className="border-r border-border bg-gradient-to-b from-blue-50 to-blue-100 text-sidebar-foreground">
        <div className="flex h-16 items-center gap-2 border-b border-border px-6 bg-gradient-to-r from-blue-600 to-blue-800">
          <ShieldIcon className="h-6 w-6 text-white" />
          <h1 className="text-lg font-semibold text-white">管理员门户</h1>
        </div>

        <div className="p-4">
          <div className="flex items-center gap-3 mb-6 bg-white p-3 rounded-lg shadow-sm">
            <Avatar>
              <AvatarImage src="/admin-avatar.png" alt="Admin" />
              <AvatarFallback className="bg-blue-600 text-white">管理</AvatarFallback>
            </Avatar>
            <div>
              <p className="font-medium">系统管理员</p>
              <p className="text-xs text-gray-500">admin@example.com</p>
            </div>
          </div>
        </div>
        <nav className="grid gap-4 px-2">
          <div className="mb-2">
            <p className="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">主要功能</p>
          </div>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-blue-100 text-blue-800 mr-2">
                <HomeIcon className="h-4 w-4" />
              </div>
              <span>首页</span>
              <Badge variant="outline" className="ml-auto text-xs">新</Badge>
            </Link>
          </Button>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/profile" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-blue-100 text-blue-800 mr-2">
                <UserIcon className="h-4 w-4" />
              </div>
              <span>个人中心</span>
            </Link>
          </Button>
          <p className="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1 mt-6">用户管理</p>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/students" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-indigo-100 text-indigo-800 mr-2">
                <UsersIcon className="h-4 w-4" />
              </div>
              <span>学生管理</span>
            </Link>
          </Button>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/resumes" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-indigo-100 text-indigo-800 mr-2">
                <FileTextIcon className="h-4 w-4" />
              </div>
              <span>简历管理</span>
            </Link>
          </Button>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/companies" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-emerald-100 text-emerald-800 mr-2">
                <BuildingIcon className="h-4 w-4" />
              </div>
              <span>企业信息</span>
            </Link>
          </Button>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/jobs" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-emerald-100 text-emerald-800 mr-2">
                <BriefcaseIcon className="h-4 w-4" />
              </div>
              <span>招聘信息</span>
            </Link>
          </Button>
          <p className="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1 mt-6">内容管理</p>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/applications" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-amber-100 text-amber-800 mr-2">
                <ClipboardIcon className="h-4 w-4" />
              </div>
              <span>应聘管理</span>
            </Link>
          </Button>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/reviews" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-amber-100 text-amber-800 mr-2">
                <StarIcon className="h-4 w-4" />
              </div>
              <span>评价管理</span>
            </Link>
          </Button>
          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/messages" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-purple-100 text-purple-800 mr-2">
                <MessageSquareIcon className="h-4 w-4" />
              </div>
              <span>留言管理</span>
            </Link>
          </Button>

          <Separator className="my-2" />

          <Button variant="ghost" className="justify-start gap-2 hover:bg-blue-200" asChild>
            <Link to="/admin/system" className="relative group">
              <span className="absolute left-0 w-1 h-6 bg-blue-600 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-gray-200 text-gray-800 mr-2">
                <CogIcon className="h-4 w-4" />
              </div>
              <span>系统管理</span>
            </Link>
          </Button>
        </nav>
      </aside>
      <main className="flex flex-col">
        <header className="flex h-16 items-center justify-between border-b border-border px-6 bg-white shadow-sm">
          <div className="flex items-center gap-3">
            <ActivityIcon className="h-5 w-5 text-blue-600" />
            <h2 className="text-lg font-semibold">管理员控制面板</h2>
            <Badge variant="outline" className="ml-2 bg-blue-50 text-blue-700 border-blue-200">系统版本 v1.2.0</Badge>
          </div>

          <div className="flex items-center gap-4">
            <Button variant="outline" size="sm" className="gap-1 text-xs">
              <ActivityIcon className="h-3 w-3" />
              系统状态
              <Badge className="ml-1 bg-green-500 text-xs">正常</Badge>
            </Button>

            <Button variant="ghost" size="icon" className="relative">
              <BellIcon className="h-5 w-5" />
              <span className="absolute top-0 right-0 h-2 w-2 rounded-full bg-red-500"></span>
              <span className="sr-only">通知</span>
            </Button>

            <Button variant="ghost" size="icon" asChild>
              <Link to="/admin/settings">
                <CogIcon className="h-5 w-5" />
                <span className="sr-only">设置</span>
              </Link>
            </Button>

            <Button variant="ghost" size="icon" asChild className="text-red-500 hover:text-red-700 hover:bg-red-50">
              <Link to="/login">
                <LogOutIcon className="h-5 w-5" />
                <span className="sr-only">登出</span>
              </Link>
            </Button>
          </div>
        </header>
        <div className="flex-1 overflow-auto p-6 bg-gray-50">
          <div className="mb-6 bg-gradient-to-r from-blue-600 to-blue-800 p-4 rounded-lg shadow-md text-white">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-xl font-bold">欢迎回来，管理员</h3>
                <p className="text-blue-100">今天是 {new Date().toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' })}</p>
              </div>
              <div className="bg-white/20 p-3 rounded-lg text-center">
                <p className="text-3xl font-bold">65</p>
                <p className="text-xs text-blue-100">今日活跃用户</p>
              </div>
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <Outlet />
          </div>
        </div>
      </main>
    </div>
  );
}
